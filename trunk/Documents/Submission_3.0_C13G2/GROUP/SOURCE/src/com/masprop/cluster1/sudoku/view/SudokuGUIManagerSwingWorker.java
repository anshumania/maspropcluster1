...
/**
 * The SudokuGUIManager 
 * 
 * @author Anshuman Mehta
 */
public class SudokuGUIManagerSwingWorker extends GUIManager implements Observer {
    ...
    public SudokuGUIManagerSwingWorker() {
//        super(gui);
    ...
    public Game getNewGame(Constraint constraint) {
        ...
        if (null == constraint) {
          ...
          startSwingWorker(constraint);

          forkGeneration(constraint);
    ...
    public void forkGeneration(final Constraint constraint)
    {
    ...
    public void startSwingWorker(final Constraint constraint)
    {
        worker = new SwingWorker() {
                public Object construct() {
                    return doWork(constraint);
                }
                public void finished() {
//                    startButton.setEnabled(true);
//                    interruptButton.setEnabled(false);
//                    statusField.setText(get().toString());
                    getGui().getGameProgressBar().setIndeterminate(false);
                    getGui().getGameProgressBar().setValue(getGui().getGameProgressBar().getMinimum());
                    getGui().getStatusText().setText("Generated " + getGeneratedSudokus() + " sudokus with more than one solution");
    ...
    public void updateStatus()
    {
            ...
            public void run() {
//                System.out.println("updatingStatus" + getGeneratedSudokus());
...
//             getGui().getGameGenerationText().setText(getGeneratedSudokus() + " invalid sudokus discarded ");
    ...
    Object doWork(Constraint constraint) {
    	 
    	
    	getGui().getGameProgressBar().setIndeterminate(true);
...
//                buttons[i][j] = new JButton(puzzle[i][j] > 0 ? String.valueOf(puzzle[i][j]) : "");
        ...
        this.getGui().getGameBoard().setBackground(Color.GRAY);
        for (int i = 0; i < N; i++) {
...
//        {
//
//                Runnable gameGeneratorThrd = new Runnable() {
//
//                    public void run() {
//                        try {
